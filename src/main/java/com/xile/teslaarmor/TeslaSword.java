...
/**
 * Created by RDAguiar on 9/26/2014.
 */
...
public class TeslaSword extends ItemSword implements IEmpowerableItem, IEnergyContainerItem {

    IIcon activeIcon;
    IIcon drainedIcon;

    public int maxEnergy = 200000;
    public int maxTransfer = 1700;
    public int energyPerUse = 210;
    public int energyPerUseCharged = 1000;

    public int damage = 9;
    public int damageTesla = 3;

    public TeslaSword(Item.ToolMaterial toolMaterial) {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase entity, EntityLivingBase player) {
        ...
        if (thePlayer.capabilities.isCreativeMode || useEnergy(stack, false) == getEnergyPerUse(stack)) {
            float teslaDamage = isEmpowered(stack) ? damageTesla : 1;
            float enchantDamage = damage + EnchantmentHelper.getEnchantmentModifierLiving(player, entity);

            entity.attackEntityFrom(DamageHelper.causePlayerFluxDamage(thePlayer), teslaDamage);
            entity.attackEntityFrom(DamageSource.causePlayerDamage(thePlayer), (teslaDamage + enchantDamage) * fallingMult);
        } else {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean check) {
        ...
        list.add(StringHelper.ORANGE + getEnergyPerUse(stack) + " " + StringHelper.localize("info.TeslaArmor.tool.energyPerUse") + StringHelper.END);
        if (isEmpowered(stack)) {
            list.add(StringHelper.YELLOW + StringHelper.ITALIC + StringHelper.localize("info.cofh.press") + " "
                    + Keyboard.getKeyName(KeyBindingEmpower.instance.getKey()) + " " + StringHelper.localize("info.TeslaArmor.tool.chargeOff")
                    + StringHelper.END);
        } else {
            list.add(StringHelper.BRIGHT_BLUE + StringHelper.ITALIC + StringHelper.localize("info.cofh.press") + " "
                    + Keyboard.getKeyName(KeyBindingEmpower.instance.getKey()) + " " + StringHelper.localize("info.TeslaArmor.tool.chargeOn")
                    + StringHelper.END);
        }
        if (getEnergyStored(stack) >= getEnergyPerUse(stack)) {
            ...
            list.add(StringHelper.BRIGHT_GREEN + "+" + (isEmpowered(stack) ? damageTesla : 1) + " " + StringHelper.localize("info.cofh.damageTesla")
                    + StringHelper.END);
        }
    }
